  ...
  codeGen X86CodeGen prg =  mapM treeX86Fun (methods prg) >>= return . (X86Prg )
    where
      ...
      treeX86Fun (Method metName _ bdy rTmp ) = do
        ...
        let epilogue = DL.fromList [
                     Binary MOV (Reg tempEAX) (Reg rTmp), --todo
        ...
        bdy' <- foldM (\is s -> munchStm s >>= return . (is `DL.append`)) DL.empty bdy
        return $ ( X86Function metName . DL.toList .
                   (`DL.append` epilogue) . (prologue `DL.append`)) bdy'
 
                    
      munchStm :: MonadNameGen m => Stm -> m (DL.DList X86Instr)
      munchStm stm =
        case stm of
          MOVE exp1 exp2 -> do
            ...
            case (op1, op2) of
              ...
              (Mem _, Mem _) -> do
                tmp <- nextTemp
                return $ ins1 `DL.append` ins2 `DL.snoc` (Binary MOV (Reg tmp) op2)
                                          `DL.snoc` (Binary MOV op1 (Reg tmp))
              _ -> return $  ins1 `DL.append` ins2 `DL.snoc` (Binary MOV op1 op2)

          Tree.LABEL label -> return (DL.singleton $ X86Instr.LABEL label)
          JUMP (NAME label) _ -> return (DL.singleton $ JMP label)
          CJUMP relOp exp1 exp2 lb1 _ -> do
            -- first op is always Reg, never Mem
            (op1, ins1) <- munchExp exp1 >>= constOrMemOpToTemp 
            (op2, ins2) <- munchExp exp2
            return $ ins1 `DL.append` ins2 `DL.snoc` (Binary CMP op1 op2) `DL.snoc` (J (sRelop relOp) lb1)
      ...
      munchExp :: MonadNameGen m => Exp -> m (Operand, DL.DList X86Instr)
      munchExp (CONST int) = return (Imm (fromIntegral int),DL.empty)
      munchExp (TEMP tmp) = return (Reg tmp, DL.empty)
      munchExp (PARAM int) = return (Mem $ EffectiveAddress (Just tempEBP)
                                     Nothing (8 + 4 * fromIntegral int), DL.empty)
      munchExp (MEM (TEMP tmp)) = return (Mem $ EffectiveAddress (Just tmp) Nothing 0, DL.empty)
      ...
      munchExp (Tree.CALL (NAME name) expS) = do
       ...
       -- Note: Arguments are pushed in the reverse order onto the stack:
        return $ (Reg tempEAX, (foldl' aux DL.empty opInsS)  `DL.snoc`
                                X86Instr.CALL name `DL.snoc`
                                -- restore stack pointer:
                                (Binary ADD (Reg tempESP) $ Imm stackP))
        where
          aux res (op, ins) = ins `DL.snoc` (Unary PUSH op) `DL.append` res
      ...
      munchExp (BINOP DIV exp1 exp2) = do
        ...
        return (Reg tempEAX, ins1 `DL.append` ins2 `DL.snoc` (Binary MOV (Reg tempEAX) op1)
                         `DL.snoc` CDQ -- sign extend EAX into EDX register
                         `DL.snoc` (Unary IDIV op2))
        
      munchExp (BINOP binOp exp1 exp2) = do 
        (op1, ins1) <- munchExp exp1 -- >>= constOpToTemp
        (op2, ins2) <- munchExp exp2
        temp <- nextTemp
        return (Reg temp, ins1 `DL.append` ins2 `DL.snoc` (Binary MOV (Reg temp) op1) `DL.snoc` (Binary xOp (Reg temp) op2))
        where
          xOp :: BinaryInstr
          xOp = do
              case binOp of
...
constOpToTemp :: (MonadNameGen m) => (Operand, DL.DList X86Instr) -> m (Operand, DL.DList X86Instr)
constOpToTemp (op, ins) =  case op of
  (Imm _) -> do
    tmp <- nextTemp
    return $ (Reg tmp, ins `DL.snoc` (Binary MOV (Reg tmp) op))
  _ -> return (op, ins)

constOrMemOpToTemp :: (MonadNameGen m) => (Operand, DL.DList X86Instr) -> m (Operand, DL.DList X86Instr)
